...
def display(companies):
    ...
    if state_data[1]!=0:
        revenue=f"${state_data[1]//1000000000:,}"
    else:
        revenue="NaN"
    if state_data[2]!=0:
        gross_profit=f"${state_data[2]//1000000000:,}"
    else:
        gross_profit="NaN"
    ...
    if not company_info.empty:
        ...
        cole.metric(f"**Revenue (in billions)**", f"{revenue}")
        colf.metric(f"**Gross Profit (in billions)**", f"{gross_profit}")
    else:
